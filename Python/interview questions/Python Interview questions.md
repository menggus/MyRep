...
4. 什么是arp协议？
   ...
   # 目的: 解析ip地址下设备的mac地址.
   # 主机发送apr包,包含目标ip地址,然后通过匹配找到ip地址的mac地址,然后再通过收到的回复消息,定位目标的地址;
   ```
...
5. TCP和UDP的区别？
   ...
   # 区别:
   1. tcp基于连接,建立连接后才能传输数据,而udp不用,直接发送;
   2. 可靠性: tcp连接,以一问一答的方式进行,发送数据有顺序,保证了数据传输的可靠性;而udp不会,只管发出,至于有没有收到并不会理会,udp可能丢包;
...
9. 什么是粘包？ socket 	中造成粘包的原因是什么？ 	哪些情况会发生粘包现象？
   ...
   # 3.对于固定长度的数据, 通过固定长度来发送数据包,而接收短,采用固定长度解析数;
...
10. tcp连接的"心跳"
    ...
    ```python
    # "心跳":用来检测连接是否断开的机制, tcp自带这种机制KEEP_ALIVE; 每隔一段时间,会定时发送0的数据包,进行检测;
...
14. 简述 进程、线程、协程的区别 	以及应用场景？
    ...
    # 特性区别: 一个程序至少有一个进程,一个进程包含一个或多个线程, 且从属多个线程共享进程的资源, 进程关闭,则线程关闭,反之不成立,也就是线程依存在进程中; 而协程相比线程为更好执行单元,线程会维护有自己的缓存,当线程切换时,会进行缓存的相关操作,使得非常耗时,而不仅仅像协程一样只是回复cpu上下文那么快;
    
    # 小结:
    1.进程资源占用大,切换进程非常耗时,效率低
    2.线程资源占用一般,切换线程耗时一般
    3.协程资源占用小,切换非常快,效率高
    4.多进程,多线程在cpu核数不同会呈现为并行或者并发, 而协成在一个线程中,为并发;
...
33. 简述 asynio模块的作用和应用场景。

    

34. 简述 gevent模块的作用和应用场景。

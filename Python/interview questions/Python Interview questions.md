...
1. 简述 OSI 	七层协议。
   ...
   # 网络层: 定义一个ip协议,同一个局域网可通过广播来传输数据,那不在一个局域网呢? 那就是通过目标ip地址转换为网关ip来获取到网关mac地址, 然后通过网关进行局域网内的数据转发;
...
17. threading.local的作用？
    ...
    ```python
    # 一个代表线程本地数据(这个本地数据可以理解为只供当前线程访问使用的数据)的类。
...
33. 什么是rpc及应用场景？
    ...
    # 它是面向过程的,而不是像http协议一样面向资源的; 通过客户端发送序列化后的通信数据,然后服务端反序列化来获取数据,然后进行服务处理, 然后客户顿 服务器交换角色, 服务器发送序列化的通信数据, 客户端反序列化来获取.

...
27. redis和memcache比较？
    ...
    # 内存使用
    redis发现内存使用量达到一定阈值会与磁盘进行swap操作, 通过计算把部分value数据放到磁盘上去(key是不会放过去的, 还需要通过key来查找数据). 这样数据可以操作内存的大小, 但是会有I/O出现.
    memcached数据全部保存在内存中, 数据大小不能超过内存大小.
    # 可靠性
    redis可做持久化, 而mencache无法做数据持久化
    # 事务支持
    memcache 在并发场景下, 通过cas来保持一致性
    redis支持事务的原子性, 一组操作要么全部执行成功, 要么全部失败
    ```
...
35. 简述redis的有哪几种持久化策略及比较？
    ...
    所以回复redis数据库一般使用, AOF日志文件进行重放,但是重放AOF比 文件快照慢很多,所以redis实例在很多情况下,恢复都很慢
